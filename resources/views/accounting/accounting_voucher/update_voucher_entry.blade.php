...
<script>
    ...
    { 
        ...
        */
        ...
        {   
            var id = $('#ac_vc_id').text();
            // showing the loader
            $('#ajax_loader_div').css('display','block');
            // api for getting new form
            $.ajax({
                url: "/accounting/voucher/update/form/"+id+'/'+ $('#voucherType').val() +'/'+formtype,
                type: "GET",
                success: function(result) {
                    ...
                    {  
                        var fix = {{$entry}};
                        var changed = $('#voucherType').val();
                        if(fix == changed){
                            $('#voucherNumber1').html(result.voucherNumber-1);
                            $('#voucherNumberElem').val(result.voucherNumber-1);
                        }else{
    ...
    {debugger;
        ...
        */
        ...
        // {
            ...
            // $(document).find('#billwise-total-amount_'+row_num).text(show_amt);
    ...
    function checkBillWise(ele)
    {
        ...
        */
        ...
        if($(ele).val()!='default' )
        {
            ...
            console.log('len',len);
            $(document).find($('#billwise-total-amount_'+id)).text(value);
            ...
            if(is_bill_wise_on==1 && len==0)                    
            {
            ...
            else if(is_bill_wise_on==0 && len==1){
                $(document).find($('#modal_div_'+id)).empty();
                // adding modal button to page
                $(document).find($('#modal_div_button_'+id)).empty();
                checkCostCenter(ele);
            }else if(is_bill_wise_on==1 && len==1){
                ...
                $(document).find($('#billwise_myModal_'+id)).on('hide.bs.modal',function(){
                    ...
                    if(is_cost_centres_on==1)
                    {
                        ...
                        {   
                            // showing modal element
                            $(document).find($('#costcenter-total-amount_'+id)).text(value);
                            $(document).find($('#costcenter-myModal_'+id)).modal('show');
                        }else
                        {
                            // creating modal element
    ...
    {
        /*
        * function to cet cost center options on change of cost category
    ...
    function checkCostCenter(ele)
    {
        ...
        */
        ...
        if($(ele).val()!='default')
        {
            ...
            }else if(is_cost_centres_on==0 && len==1){
                $(document).find($('#modal_div_'+id)).empty();
                // adding modal button to page
                $(document).find($('#modal_div_button_'+id)).empty();
            }   
    ...
    function checkCostCenterTax(ele)
    {
        ...
        if($(ele).val()!='default')
        {
            ...
            if(is_cost_centres_on==1 && len==0)                    
            {
                ...
                $(document).find($('#modal_tax_div_'+id)).empty();
                // adding modal button to page
                $(document).find($('#modal_tax_div_button_'+id)).empty();

            }else if(is_cost_centres_on==1 && len==1){

                $(document).find($('#costcenter-myModal_'+id)).modal({
                    backdrop: 'static',
                    show: true
                });
    ...
    function addrow(id)
    {
        ...
        */
        ...
        var newstr = '_'+id+'"';
        // var type= $(document).find('#amount_type_0').val();
        // var acc ='account_'+type+'';
        // var accrep ='account_'+type+'[]';
        // var amt ='amount_'+type+'';
        // var amtrep ='amount_'+type+'[]';
        // replacing the pattern with new id
        var res =  ele.replace(patt,newstr).replace('HIDDEN',' ');
        // .replace('amount_type','amount_type[]').replace(acc,accrep).replace(amt,amtrep).replace('narration','narration[]');
        // adding the new element to page
        ...
        <div class="box-header with-border">
            <div class='box box-default'>  <br>
                <div class="row">
                    ...
                    <form method="POST" novalidate  action="/accounting/voucher/update/{{$id}}" id="voucher_entry_form">
                        <p id="ac_vc_id" hidden>{{$id}}</p>
                        <div class="col-md-5">
                            <button class="btn bg-red btn-sm" onclick="p_dated()">Post Dated</button>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <div class="row">
                                                ...
                                                <div class="col-md-8">
                                                    <input type="text" class="form-control input-css datepicker1" value="{{ date('Y-m-d',strtotime($selectedvoucher->date)) }}" name="date" id="current_date">
                                                </div>
                                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <select class="select input-css form-control voucherType" name="voucherType" id="voucherType">
                                        ...
                                        @foreach ($totalvouchers as $key)
                                            <option value="{{$key->id}}" {{$entry==$key->id?'selected':''}}>{{$key->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select class="select input-css form-control form_type" name="form_type" id="form_type">
                                        ...
                                        <option class="form_type_class" value="0" {{$vc_type==0?'selected':''}}>As Voucher</option>
                                        <option class="form_type_class" value="1" {{$vc_type==1?'selected':''}}>As Invoice</option>
                                        <option class="form_type_class" value="2" {{$vc_type==2?'selected':''}}>As Journal</option>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4" style="text-align:right;">
                                    <div id="post_dated_enabled_div" style="display:none">
                                        ...
                                        <div class="col-md-8">
                                        <input type="text" class="form-control input-css datepicker1" value="{{date('Y-m-d',strtotime($selectedvoucher->date))}}"  name="voucherDate" id="voucherDate">
                                    </div>
                                </div>
                                <input type="hidden" name="is_post_dated" value="0" id="is_post_dated">
                                <div id="post_dated_disabled_div" style=" display:inline-block">
                                            {{__('accounting/voucher.date')}}
                                            {{date('l',strtotime($selectedvoucher->date))}}, {{date('d-M-Y',strtotime($selectedvoucher->date))}}
                                    </div>
                                </div>
                                <div class="row">
                                    {{-- effective date div --}}
                                        <div style="text-align:right;display:none;" class="col-md-12 effectiveDateDiv" >Effective Date : {{$selectedvoucher->effective_date}}</div>
                                </div>
                            </div>
