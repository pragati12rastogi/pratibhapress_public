...
@section('title', __('gatepass.title2'))
...
$('#gatepass_form').on('submit', function(event) {
    ...
    $('.challan_num').each(function(e) { 
        $(this).rules("add",{ 
                required: true,
                messages: { required: "Delivery Challan Number is required"}
        });    
    });
            ...
            <div class="box-header with-border">
                <div class='box box-default'>  <br> 
                    ...
                    <div class="container-fluid wdt">
                        <form method="POST" action="/gatepass/returnable/insert" id="gatepass_form">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                                        <label for="challan_type">{{__('gatepass.challan_type')}}</label>
                                        <div class="col-md-6">
                                            <label><input checked type="radio" checked value="delivery_challan" class="challan_type" id="challan_type" name="challan_type">
                                                {{__('gatepass.challan')}}</label>
                                        </div>
                                        <div class="col-md-6">
                                            <label><input type="radio" class="challan_type" value="internal_dc" id="challan_type" name="challan_type">
                                                {{__('gatepass.ichallan')}}</label>
                                        </div>
                                    </div>
                                    <div class="col-md-6" style="display:none" id="internal_dc_div">
                                        <label for="">{{__('gatepass.ichallan')}}<sup>*</sup></label>
                                        <select value="{{ old('challan_num') }}"  name="ichallan_num" class="select2 input-css ichallan_num" id="ichallan_num">
                                            <option value="">Select Challan Number</option>
                                            @foreach ($internal_dc as $item)
                                            <option value="{{$item['id']}}">{{$item['idc_number']}}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-6" id="delivery_challan_div">
                                        ...
                                        <select value="{{ old('challan_num') }}" name="challan_num" class="select2 input-css challan_num" id="challan_num">
                                            ...
                                            <option value="{{$item['id']}}">{{$item['challan_number']}}</option>
                                            @endforeach
                                        </select>
                                        <label id="challan_num-error" class="error" for="challan_num"></label>
                                        {!! $errors->first('challan_num', '<p class="help-block">:message</p>') !!}
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="remark" id="remark" class="input-css remark">
                                        {!! $errors->first('remark', '<p class="help-block">:message</p>') !!}
                                    </div>
                                    <div class="col-md-6">
                                        <label for="">{{__('gatepass.return')}}<sup>*</sup></label>
                                        <input type="text" name="return" id="return" class="input-css return datepicker1">
