...
@section('main_section')
    <section class="content">
        ...
        <form method="POST" action="/binder/update/{{$id}}" id="vendor" enctype="multipart/form-data">
                   @csrf
                   ...
                   <div class="box-header with-border">
                    ...
                    <div class="row">
                        <div class="col-md-6 {{ $errors->has('aadhar_file') ? 'has-error' : ''}}">
                            ...
                            @if($item->aadhar_file != "" || $item->aadhar_file != null)
                            
                                @if (file_exists(public_path().'/upload/binder/aadhar/'.$item->aadhar_file ))
                                    <?php $ext = pathinfo(storage_path().'/upload/binder/aadhar/'.$item->aadhar_file, PATHINFO_EXTENSION); ?>
                                    @if ($ext == 'pdf')
                                        <a href="{{ asset('upload/binder/aadhar/')}}/{{$item->aadhar_file}}" target="_blank">See Aadhar File</a>
                                    @else
                                        <img src="{{ asset('upload/binder/aadhar/')}}/{{$item->aadhar_file}}" height="50" width="100">
                                    @endif
                                @endif
                            @endif
                            <input type="file" class="" name="aadhar_file">
                            <input type="text" name="old_image" value="{{$item->aadhar_file}}" hidden>
                        ...
                        <div class="col-md-6 {{ $errors->has('aadhar_no') ? 'has-error' : ''}}">
                            ...
                            <input type="text" name="aadhar_no" class="form-control input-css" required="" maxlength="12"  min="12" value="{{$item->aadhar_no}}">
                    ...
                    <div class="row">
                        <div class="col-md-6 {{ $errors->has('contract_file') ? 'has-error' : ''}}">
                            ...
                            @if($item->contract_file != "" || $item->contract_file != null)
                            
                                @if (file_exists(public_path().'/upload/binder/contract/'.$item->contract_file ))
                                    <?php $ext = pathinfo(storage_path().'/upload/binder/contract/'.$item->contract_file, PATHINFO_EXTENSION); ?>
                                    @if ($ext == 'pdf')
                                        <a href="{{ asset('upload/binder/contract/')}}/{{$item->contract_file}}" target="_blank">See Aadhar File</a>
                                    @else
                                        <img src="{{ asset('upload/binder/contract/')}}/{{$item->contract_file}}" height="50" width="100">
                                    @endif
                                @endif
                            @endif
                            <input type="file" class="" name="contract_file" >
                            <input type="text" name="old_bill" value="{{$item->contract_file}}" hidden>
