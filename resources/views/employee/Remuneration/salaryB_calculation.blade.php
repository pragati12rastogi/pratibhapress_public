...
@section('title', 'Salary B Calculation')
 
@section('user', Auth::user()->name)
 
@section('breadcrumb')
    <li><a href="#"><i class=""></i> Salary A Calculation</a></li>
...
var month=date.getMonth(); //get month
    ...
    function getData(counter){
        ...
        var total_salaryB=result.employee.total_salaryB;
        var overtime_salaryB=result.employee.overtime_salaryB;
        ...
        if(counter==1){
            ...
            if(parseFloat(ad)>parseFloat(balance_advance)){
                alert('Advance Deducted is greater than Balance Advance. It must be less than or equal to balance advance');
                getData(0);
            }
       ...
       console.log("ad = "+ad);
        ...
        $('.tot_sal').text(total_salaryB);
        $('.ot_A').text(overtime_salaryB);
       ...
       if(flag==0 && mons[0]!=month){
        ...
        var half_day=(parseFloat(total_salaryB)/parseFloat(total_days))*parseFloat(hd);
        $('.hd_deduction').text(half_day.toFixed(2));
        $('.hd_deduction').val(half_day.toFixed(2));

        $('.od').text(other_time_deduction);
        var other_deduction=((parseFloat(total_salaryB)/parseFloat(total_days/360))*parseFloat(other_time_deduction));
        ...
        //effective absent
        if(parseInt(total_a)<=2){
            var effective_absent=0;
        }
        else if(parseInt(total_a)>2){
            var effective_absent=parseInt(total_a);
        }
        ...
        $('.effective_absent').val(effective_absent);
         ...
         //effective present
         var effective_present=parseFloat(total_days)-parseFloat(effective_absent);
        ...
        var salary_ctc=(parseFloat(total_salaryB)/parseFloat(total_days))*parseFloat(effective_present);
        ...
        var ot_ctc=parseFloat(parseFloat(overtime_salaryB)/parseFloat(total_days)/360)*ot_late;
        ...
        var salary=parseFloat(salary_ctc)+parseFloat(ot_ctc)-parseFloat(ad)-parseFloat(other_deduction)-parseFloat(half_day);
...
@section('main_section')
    <section class="content">
        ...
        <!-- Default box -->
        <form action="/salaryB/create" method="post">
        @csrf
                ...
                <input type="hidden" name="month" class="mon">
                        <div class="col-md-12">
                                <table class="table table-bordered table-striped">
                                        ...
                                        <tr>
                                            <th colspan="3">Total Salary B:</th>
                                            <td ><label for="" class="tot_sal"></td>
                                            
                                            <th colspan="3">OT on salary B:</th>
                                            <td><label for="" class="ot_A"></td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            ...
                                            <th colspan="3">Salary calculated:</th>
                                            <td><input type="hidden" name="salary_ctc" class="salary_ctc input-css"><label class="salary_ctc"></label></td>
                                        
                                            <th colspan="3">Advance balance after deduction from Salary A:</th>
                                            <td><label for="" class="advance_bal"></td>
