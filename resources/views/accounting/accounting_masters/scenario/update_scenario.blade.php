...
@section('title', __('accounting/scenario.update title'))

{{-- TODO: fetch from auth --}}
@section('user', Auth::user()->name)

@section('breadcrumb')
<li><a href="#"><i class="">{{__('accounting/scenario.update title')}}</i></a></li>
 @endsection
@section('css')
<style>
            ...
            <div class="box-header with-border">
                <div class='box box-default'>  <br>
                    <div class="container-fluid wdt">
                        <form method="POST" action="/accounting/scenario/update/{{$sd->id}}" id="asn_form">
                            @csrf
                            <div class="row">
                                <div class="col-md-6">
                                    <label for="">{{__('accounting/scenario.name')}}</label>
                                    <input type="text" name="name" value="{{$sd->name}}" id="name" class="input-css form-control name">
                                    <input type="hidden" name="id" value="{{$sd->id}}">
                                  </div>
                                @if ($show_alias==1)                                    
                                    <div class="col-md-6">
                                        <label for="">{{__('accounting/scenario.alias')}}</label>
                                        <input type="text" name="alias" value="{{$sd->alias}}" id="alias" class="input-css form-control alias">
                                    </div>
                                @endif
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <select  name="include_actuals" id="include_actuals" class="select input-css form-control include_actuals">
                                        <option value="0" {{$sd->include_actuals == 0? 'selected="selected"':''}}>No</option>
                                        <option value="1" {{$sd->include_actuals == 1? 'selected="selected"':''}}>Yes</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <select  name="exclude_forex" id="exclude_forex" class="select input-css form-control exclude_forex">
                                        <option value="0" {{$sd->exclude_forex == 0? 'selected="selected"':''}}>No</option>
                                        <option value="1" {{$sd->exclude_forex == 1? 'selected="selected"':''}}>Yes</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row">
