...
@section('main_section')
    <section class="content">
        ...
        <!-- Default box -->
        <form action="/design/work/update/{{$design_allotment->id}}" method="POST" id="form">
        @csrf
        <div class="box-header with-border returnable">
                <div class='box box-default'>  <br>
                        <h2 class="box-title" style="font-size: 28px;margin-left:20px">Update Design Work Allotment</h2><br><br><br>
                        ...
                        <div class="row">
                                ...
                                <div class="row">
                                        <div class="col-md-6 {{ $errors->has('design') ? 'has-error' : ''}}">
                                            ...
                                            <select name="designorder" disabled id="design" class="select2 design input-css">
                                                            <option value="">Select Design Order</option>
                                                            @foreach ($design as $item)
                                                <option value="{{$item->id}}" {{$design_allotment->design_id==$item->id ? 'selected=selected':''}}>{{$item->do_number}}</option>
                                                        @endforeach
                                            </select>
                                        <input type="hidden" name="design" value="{{$design_allotment->design_id}}">
                                        ...
                                        <div class="col-md-6 {{ $errors->has('work_to_emp') ? 'has-error' : ''}}">
                                                ...
                                                <select name="work_to_emp" id="work_to_emp" class="select2 work_to_emp input-css">
                                                        ...
                                                        @foreach ($emp as $item)
                                            <option value="{{$item->id}}" {{$design_allotment->work_emp_id==$item->id ? 'selected=selected':''}}>{{$item->name}}</option>
                                                    @endforeach
                                        </select>
                                ...
                                <div class="row">
                                       {{-- <!-- <div class="col-md-6 {{ $errors->has('work_date') ? 'has-error' : ''}}">
                                        ...
                                        <input type="text" name="work_date" value="{{CustomHelpers::showDate($design_allotment->work_date,'d-m-Y')}}" id="work_date" class="datepicker1 work_date input-css">
                                        ...
                                        <input type="number" name="no_pages" value="{{$design_allotment->no_pages}}" max="{{$design_allotment->no_pages+$design_allotment->left_pages}}" placeholder="Max:{{$design_allotment->no_pages+$design_allotment->left_pages}}" id="no_pages" class="no_pages input-css">
                                        <input type="hidden" name="left_pages" value="{{$design_allotment->left_pages}}">
                                        <input type="hidden" name="old_pages" value="{{$design_allotment->no_pages}}">
                                        ...
                                        <div class="row">
                                                <div class="col-md-12 {{ $errors->has('desc') ? 'has-error' : ''}}">
                                                        ...
                                                        <textarea name="desc" id="desc" class="desc input-css">{{$design_allotment->description}}</textarea>
